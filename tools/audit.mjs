...
await page.screenshot({ path: `${OUT}/shift-typed.png` });

// reset
for (let i = 0; i < 6; i++) { await page.click('.numdisp__back'); await page.waitForTimeout(60); }
...
async function auditScreen(name) {
  ...
  } else log('  touch targets: all ≥44px');
}
